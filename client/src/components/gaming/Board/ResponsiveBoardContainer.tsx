...
export const ResponsiveBoardContainer: React.FC<ResponsiveBoardContainerProps> = ({
  ...
  useEffect(() => {
    const calculateSize = () => {
      ...
      // Account for 1.421:1 aspect ratio of board squares (rectangular cells, +1.5% taller)
      let baseSize = Math.min(
        width * 0.99, // ULTRA MAXIMIZED space utilization
        (height - 20) * 1.0 / 1.421, // Divide by 1.421 to account for taller rectangular cells (+1.5%), ultra minimal margin
        maxSize
      );
      ...
      switch (breakpoint) {
        ...
        case 'tablet':
          ...
          break;
        case 'desktop':
          baseSize = Math.min(baseSize, width * 0.97, height * 1.0 / 1.421, 1500); // Ultra maximum desktop utilization (+1.5% taller)
          break;
        case '2k':
          // Optimized sizing for 2K displays - better space utilization
          baseSize = Math.min(
            width * 0.85, // Ultra maximum 2K space utilization
            height * 0.92 / 1.421, // Divide by 1.421 for taller rectangular cells (+1.5%)
            1700 // Ultra maximum 2K size
          );
          break;
        case 'ultrawide':
          baseSize = Math.min(baseSize, width * 0.4, height * 0.75 / 1.421, 1000); // (+1.5% taller)
          break;
      }
  ...
  return (
    <div className={getContainerClasses()}>
      <motion.div
        ...
        style={{
          width: containerSize,
          height: containerSize * 1.421, // Height accounts for 1.421:1 aspect ratio (+1.5% taller)
          maxWidth: '100%',
          maxHeight: '100%',
        }}
      >
