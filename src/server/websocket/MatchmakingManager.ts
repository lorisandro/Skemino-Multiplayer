...
export class MatchmakingManager extends EventEmitter {
  ...
  public async addToQueue(player: MatchmakingPlayer): Promise<Match | null> {
    try {
      ...
      logger.info(
        `🔍 MatchmakingManager.addToQueue: Player ${player.username} requesting ${timeControl} queue`,
      );
      ...
      logger.info(
        `📊 Queue ${timeControl} current state: ${queue.players.size} players waiting`,
      );
      if (queue.players.size > 0) {
        const waitingPlayers = Array.from(queue.players.values())
      ...
      logger.info(
        `⏳ Player ${player.username} (${player.rating}) joined ${timeControl} queue - Total: ${queue.players.size} players`,
      );
      ...
      if (match) {
        logger.info(
          `✨ Immediate match created: ${match.white.username} vs ${match.black.username}`,
        );
        await this.removePlayersFromQueue(timeControl, [
          match.white.userId,
          match.black.userId,
        ]);
        return match;
        ...
        logger.info(
          `❌ No immediate match found for ${player.username} in ${timeControl} queue`,
        );
      }
  ...
  private tryCreateMatch(
    ...
    newPlayer: MatchmakingPlayer,
  ): Match | null {
    logger.info(
      `🔍 tryCreateMatch: Looking for match for ${newPlayer.username} in ${queue.timeControl} queue`,
    );
    ...
    for (const player of queue.players.values()) {
      if (player.userId !== newPlayer.userId) {
        ...
        logger.info(
          `👤 Found potential opponent: ${player.username} (${player.rating})`,
        );
      }
    }
    ...
    logger.info(
      `🎯 Found ${candidates.length} potential opponents for ${newPlayer.username}`,
    );

    // Sort candidates by rating compatibility and wait time
    const rankedCandidates = this.rankCandidates(newPlayer, candidates);
    logger.info(
      `📊 Ranked candidates: ${rankedCandidates.map((c) => `${c.username}(${c.rating})`).join(", ")}`,
    );

    for (const candidate of rankedCandidates) {
      logger.info(
        `🤝 Checking match compatibility: ${newPlayer.username}(${newPlayer.rating}) vs ${candidate.username}(${candidate.rating})`,
      );
      ...
      if (this.canPlayersMatch(newPlayer, candidate)) {
        ...
        logger.info(
          `❌ Players cannot match - rating difference or other constraints`,
        );
      }
    }

    logger.info(
      `❌ No compatible match found for ${newPlayer.username} after checking all candidates`,
    );
    return null;
  }
  ...
  private calculatePreferenceScore(
    ...
    // Check avoid lists
    if (
      player1.preferences?.avoidOpponents?.includes(player2.userId) ||
  ...
  private canPlayersMatch(
    ...
    const maxRatingDiff =
      this.INITIAL_RATING_RANGE + expansions * this.RATING_RANGE_EXPANSION;

    // Check rating difference
    const ratingDiff = Math.abs(player1.rating - player2.rating);

    logger.info(
      `🔢 Rating compatibility check: ${player1.username}(${player1.rating}) vs ${player2.username}(${player2.rating})`,
    );
    logger.info(
      `📊 Rating difference: ${ratingDiff}, Max allowed: ${maxRatingDiff} (wait time: ${waitTime}ms, expansions: ${expansions})`,
    );

    if (ratingDiff > maxRatingDiff) {
      logger.info(
        `❌ Rating difference too large: ${ratingDiff} > ${maxRatingDiff}`,
      );
      return false;
    }
    ...
    logger.info(
      `⚙️ Custom rating preferences: P1 max: ${p1MaxDiff}, P2 max: ${p2MaxDiff}`,
    );

    if (ratingDiff > p1MaxDiff || ratingDiff > p2MaxDiff) {
      logger.info(
        `❌ Rating difference exceeds player preferences: ${ratingDiff} > ${Math.min(p1MaxDiff, p2MaxDiff)}`,
      );
      return false;
    }
  ...
  private createMatch(
    ...
    logger.info(
      `🎮 Match created: ${white.username} (${white.rating}) vs ${black.username} (${black.rating}) - ${match.timeControl}`,
    );
  ...
  private assignColors(
    player1: MatchmakingPlayer,
    player2: MatchmakingPlayer,
  ): { white: MatchmakingPlayer; black: MatchmakingPlayer } {
    const p1ColorPref = player1.preferences?.preferredColor;
    const p2ColorPref = player2.preferences?.preferredColor;
    ...
    if (Math.abs(player1.rating - player2.rating) > 100) {
      ...
      if (Math.random() < 0.6) {
        // 60% chance higher-rated plays white
  ...
  private async removePlayersFromQueue(
    timeControl: string,
    userIds: string[],
  ): Promise<void> {
  ...
  private async updateQueueInRedis(
    timeControl: string,
    queue: MatchmakingQueue,
  ): Promise<void> {
    try {
  ...
  private async processAllQueues(): Promise<void> {
    ...
    if (totalPlayers > 0) {
      logger.info(
        `🔍 Processing queues: ${totalPlayers} total players waiting`,
      );
      ...
      for (const [timeControl, queue] of this.queues) {
        if (queue.players.size > 0) {
          const players = Array.from(queue.players.values())
            ...
            .join(", ");
          logger.info(
            `📊 Queue ${timeControl}: ${queue.players.size} players - ${players}`,
          );
        }
      }
    }
    ...
    for (const [timeControl, queue] of this.queues) {
      if (queue.players.size >= 2) {
        logger.info(
          `⚡ Processing ${timeControl} queue: ${queue.players.size} players ready for matching`,
        );
  ...
  private async processQueue(
    ...
    for (let i = 0; i < players.length - 1; i++) {
      ...
      if (queue.players.has(player1.userId)) {
        // Player still in queue
        for (let j = i + 1; j < players.length; j++) {
          const player2 = players[j];

          if (
            queue.players.has(player2.userId) &&
            this.canPlayersMatch(player1, player2)
          ) {
    ...
    if (matches.length > 0) {
      ...
      matches.forEach((match) => {
        ...
        logger.info(
          `🔄 Processed queue match: ${match.white.username} vs ${match.black.username} (${timeControl})`,
        );
      });
    }
  }
  ...
  public getQueueStats(): {
    ...
    for (const [timeControl, queue] of this.queues) {
      ...
      stats[timeControl] = {
        ...
        avgWaitTime:
          players.length > 0
            ? players.reduce((sum, p) => sum + (now - p.joinedAt), 0) /
              players.length
            : 0,
        avgRating:
          players.length > 0
  ...
  public getPlayerPosition(
    userId: string,
  ): { timeControl: string; position: number; estimatedWait: number } | null {
    for (const [timeControl, queue] of this.queues) {
      const player = queue.players.get(userId);
      if (player) {
        const players = Array.from(queue.players.values()).sort(
  ...
  public getTotalPlayersInQueues(): number {
    return Array.from(this.queues.values()).reduce(
      (total, queue) => total + queue.players.size,
      0,
    );
  }
    ...
    for (const [timeControl, queueData] of Object.entries(status)) {
      ...
      if (queueData.playerCount > 0) {
        queueData.players.forEach((player: any, index: number) => {
          logger.info(
            `   ${index + 1}. ${player.username} (${player.rating}) - waiting ${Math.round(player.waitTime / 1000)}s`,
          );
        });
      }

      logger.info(
        `   Last match: ${Math.round(queueData.queueAge / 1000)}s ago`,
      );
      logger.info("");
    }

    logger.info(
      `👥 Total players across all queues: ${this.getTotalPlayersInQueues()}`,
    );
