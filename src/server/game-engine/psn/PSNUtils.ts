...
export class PSNUtils {
  ...
  public async loadGamesFromDirectory(
    ...
    const psnFiles = files.filter(
      (file) => extname(file).toLowerCase() === ".psn",
    );
   ...
   */
  public async validatePSNArchive(
    directoryPath: string,
  ): Promise<PSNValidationReport> {
    const files = await fs.readdir(directoryPath);
    const psnFiles = files.filter(
      (file) => extname(file).toLowerCase() === ".psn",
    );
   ...
   */
  public async generateArchiveStats(
    directoryPath: string,
  ): Promise<PSNArchiveStats> {
   ...
   */
  public repairPSNString(psnString: string): {
    repaired: string;
    changes: string[];
  } {
    ...
    repaired = repaired.replace(
      ...
      (match, turn, suit, value, position) => {
        changes.push(
          `Fixed move notation format: ${match} -> ${turn}.${suit}${value}:${position}`,
        );
    ...
    repaired = repaired.replace(
      ...
      (match, cardPart, position) => {
        changes.push(
          `Added missing colon: ${match} -> ${cardPart}:${position}`,
        );
   ...
   */
  public validateMoveSequence(moves: Move[]): {
    isValid: boolean;
    errors: string[];
  } {
    const errors: string[] = [];
    ...
    for (let i = 0; i < moves.length; i++) {
      ...
      if (moves[i].turnNumber !== expectedTurn) {
        errors.push(
          `Move ${i + 1}: Expected turn ${expectedTurn}, got ${moves[i].turnNumber}`,
        );
      }

      if (moves[i].player !== expectedPlayer) {
        errors.push(
          `Move ${i + 1}: Expected player ${expectedPlayer}, got ${moves[i].player}`,
        );
      }
    }
    ...
    for (let i = 0; i < moves.length; i++) {
      ...
      if (positionHistory.has(position)) {
        ...
        errors.push(
          `Move ${i + 1}: Position ${position} was already occupied by move ${previousMove}`,
        );
      }
  ...
  public getMoveAtTurn(
    ...
    return (
      moves.find(
        (move) => move.turnNumber === turn && move.player === player,
      ) || null
    );
  }
