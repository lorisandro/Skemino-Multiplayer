...
export class ELOCalculator {
   ...
   */
  public static calculateKFactor(rating: number, gamesPlayed: number = 0): number {
    ...
    const baseK = this.BASE_K_FACTOR * Math.exp(-rating / this.K_DECAY_CONSTANT);

    // Apply bounds
    let kFactor = Math.max(this.MIN_K_FACTOR, Math.min(this.MAX_K_FACTOR, baseK));
  ...
  public static calculateNewRating(
    ...
    const expectedScore = this.calculateExpectedScore(currentRating, opponentRating, kFactor);
  ...
  public static calculateMatchRatings(
    ...
    const whiteExpected = this.calculateExpectedScore(whiteRating, blackRating, whiteKFactor);
    const blackExpected = this.calculateExpectedScore(blackRating, whiteRating, blackKFactor);
   ...
   */
  public static calculateRatingReliability(rating: number, gamesPlayed: number): number {
    if (gamesPlayed < this.PROVISIONAL_GAMES) {
  ...
  public static getSkillLevel(rating: number): {
    ...
    const levels = [
      { level: 'Beginner', tier: 1, description: 'Learning the basics', min: 100, max: 1199 },
      { level: 'Novice', tier: 2, description: 'Understanding fundamentals', min: 1200, max: 1399 },
      { level: 'Intermediate', tier: 3, description: 'Developing strategy', min: 1400, max: 1599 },
      { level: 'Advanced', tier: 4, description: 'Strong tactical play', min: 1600, max: 1799 },
      { level: 'Expert', tier: 5, description: 'Highly skilled', min: 1800, max: 1999 },
      { level: 'Master', tier: 6, description: 'Tournament level', min: 2000, max: 2199 },
      { level: 'International Master', tier: 7, description: 'Elite player', min: 2200, max: 2399 },
      { level: 'Grand Master', tier: 8, description: 'World-class', min: 2400, max: 2599 },
      { level: 'Super Grand Master', tier: 9, description: 'Among the best', min: 2600, max: 2799 },
      { level: 'World Champion Level', tier: 10, description: 'Legendary', min: 2800, max: 3000 }
    ];
  ...
  public static calculatePairingStrength(
    ...
    const kFactor = this.calculateKFactor(avgRating, Math.max(player1Games, player2Games));

    const expectedBalance = this.calculateExpectedScore(player1Rating, player2Rating, kFactor);
