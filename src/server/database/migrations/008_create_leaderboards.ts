...
export async function up(knex: Knex): Promise<void> {
  ...
  await knex.schema.createTable('leaderboards', (table) => {
    ...
    table.enum('period', ['daily', 'weekly', 'monthly', 'yearly', 'all_time']).notNullable();
    table.enum('category', [
      'rating',
      'games_won',
      'win_streak',
  ...
  await knex.schema.createTable('achievements', (table) => {
    ...
    table.enum('category', ['gameplay', 'social', 'competitive', 'special']).notNullable();
    table.enum('rarity', ['common', 'uncommon', 'rare', 'epic', 'legendary']).notNullable();
